...
export type DropCallbacks = {
  ...
  onPlace?: (p: { id: string; type: string; position: THREE.Vector3; rotation: THREE.Euler; scale: THREE.Vector3; }) => void;
};

export function useTilesDnD(groupName = "tiles-root", { onGhost, onPlace }: DropCallbacks) {
  const { camera, scene, gl } = useThree();
  const raycaster = useMemo(() => new THREE.Raycaster(), []);
  const mouse = useMemo(() => new THREE.Vector2(), []);
  const getGroup = () => scene.getObjectByName(groupName);
  ...
  }, [camera, scene, gl, onGhost, onPlace, raycaster, mouse, groupName]);
}