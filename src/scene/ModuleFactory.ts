...
export function createModuleMesh(type: string): THREE.Mesh {
  ...
  const colors: Record<ModuleKind, number> = {
    crew: 0xf3c623, hygiene: 0x2db37d, lab: 0x5a8dee, eclss: 0xc061cb, default: 0xd46a3a,
  };
  const mat = new THREE.MeshStandardMaterial({ color: colors[kind], roughness: 0.7, metalness: 0.0 });
  const mesh =
    kind === "hygiene" ? new THREE.Mesh(new THREE.CylinderGeometry(6, 6, 8, 32), mat) :
    (kind === "lab" || kind === "eclss") ? new THREE.Mesh(new THREE.BoxGeometry(20, 12, 14), mat) :
    new THREE.Mesh(new THREE.BoxGeometry(16, 10, 16), mat);
